  ...
  return (
    <Container maxWidth='sm' sx={{ padding: '1rem' }}>
      ...
      <Stack direction={'row'} spacing={'1rem'}>
        ...
        <Button sx={{ borderRadius: '50%', color: 'green' }} onClick={audioHandler}><VolumeUp /></Button>
      </Stack>
      <Button sx={{ margin: '3rem 0', fontWeight: 'bold' }} variant="contained" fullWidth
        onClick={count === words.length-1 ? () => navigate('/quiz') : nextHandler}>{count === words.length-1 ? "Test" : "Next"}</Button>
    </Container>
  )
}
